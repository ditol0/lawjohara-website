...
<head>
    ...
    <title>{{ $title ?? 'Al Jawhara Law Firm and Legal Consultancy' }}</title>
    <meta name="description" content="{{ $description ?? 'Integrated legal solutions for individuals and businesses in Saudi Arabia.' }}">
...
<body class="{{ $bodyClass ?? '' }} en-layout">
    @include('layout.partials.main-sidebar-en', ['page' => $page ?? 'home'])
    <div class="site-overlay" id="siteOverlay"></div>

    @include('layout.partials.main-header-en', ['page' => $page ?? 'home'])

    <main class="site-main">
        @yield('content')
    </main>

    @include('layout.partials.main-footer-en')

    <a href="https://wa.me/+966533461460" class="floating-whatsapp" target="_blank" rel="noopener" aria-label="Contact us on WhatsApp">
